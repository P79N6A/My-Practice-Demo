...
function methods () {
  ...
  self.updateCanvas = function (done) {
    if (self.croperTarget) {
      ...
      console.log(`left: ${self.imgLeft} top: ${self.imgTop}`);
    }
    tools_7(self.onBeforeDraw) && self.onBeforeDraw(self.ctx, self);

    self.setBoundStyle(self.boundStyle); //	设置边界样式（设置 canvas 的大小边框
...
var getNewScale = function (oldScale, oldDistance, zoom, touch0, touch1) {
  ...
  // 计算二指最新距离
  xMove = Math.round(touch1.x || touch1.pageX - touch0.x || touch0.pageX);
  yMove = Math.round(touch1.y || touch1.pageX - touch0.y || touch0.pageX);
...
function update () {
  ...
  self.__oneTouchStart = function (touch) {
    self.touchX0 = Math.round(touch.x || touch.pageX);
    self.touchY0 = Math.round(touch.y || touch.pageY);
  };
  ...
  self.__oneTouchMove = function (touch) {
    ...
    console.log(touch)
    console.log(`touchX: ${touch.x}`)
    xMove = Math.round(touch.x || touch.pageX - self.touchX0);
    console.log(`xMove: ${xMove}`)
    yMove = Math.round(touch.y || touch.pageY - self.touchY0);

    var imgLeft = Math.round(self.rectX + xMove);
    var imgTop = Math.round(self.rectY + yMove);

    console.log(`imgL: ${imgLeft}`)
  ...
  self.__twoTouchStart = function (touch0, touch1) {
    ...
    // 计算两指距离
    xMove = Math.round(touch1.x || touch1.pageX - touch0.x || touch0.pageX);
    yMove = Math.round(touch1.y || touch1.pageY - touch0.y || touch0.pageY);
